...
class CIUpdate:
    ...
    def run_update(self):
        ...
        log.info("Running optional update from kafka")
    ...
    def update_kafkatest(self):
        ...
        dev_version = "{}.dev0".format(self.new_version.split("-ce")[0])
        ...
        ducktape_version = self.new_version.split("-ce")[0]
    ...
    def update_quickstart(self):
        ...
        build_number = self.new_version.split("-")[1]
        quickstart_pom = os.path.join(self.repo_path, "streams/quickstart/pom.xml")
        quickstart_version = self.get_version_maven(quickstart_pom)
        new_quickstart_version = "{}-{}".format(quickstart_version, build_number)
        self.update_project_versions(quickstart_pom, new_quickstart_version)
        ...
        self.regexReplace(archetype_resources_pom,
                          ...
                          "<kafka.version>{}</kafka.version>".format(new_quickstart_version))
    ...
    def get_version_maven(self, pom_file):
        """Parse the project root pom file and extract the project version."""
        log.info("Getting current project version from pom file.")
        pom = ET.ElementTree()
        pom.parse(pom_file)
        version_element = pom.getroot().find("{}version".format(NAME_SPACE))

        if version_element is not None:
            current_version = version_element.text.strip()
            # Strip off the build number or any other qualifier that may be present
            current_version = current_version.split("-")[0]
            log.info("Current project version: {}".format(current_version))
            return current_version
        else:
            log.error("Could not find project version in pom file.")
            sys.exit(1)
    ...
    def update_project_versions(self, pom_file, new_version):
        ...
        cmd = "mvn versions:set -DnewVersion={} ".format(new_version)
    ...
    def update_project_property(self, pom_file, property_name, new_value):
        """Update a property in the pom file."""
        cmd = "mvn versions:set-property "
        cmd += "-DgenerateBackupPoms=false "
        cmd += "-Dproperty={} ".format(property_name)
        cmd += "-DnewVersion={} ".format(new_value)
        cmd += "-f {}".format(pom_file)
        log.info("Updating property: {}".format(property_name))

        if self.run_cmd(cmd):
            log.info("Finished updating the property.")
        else:
            log.error("Failed to update the property.")
            sys.exit(1)
